...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><?php if($data["title"] != '' ): ?><a class="back" href='<?php echo U("Nav/index/pid/$data[pid]");?>' title="返回上级分类列表"><i class="fa fa-arrow-circle-o-left"></i></a><?php endif; ?>
      <div class="subject">
        <h3>类目管理<?php if($data["title"] != '' ): ?>- "<?php echo ($data["title"]); ?>"的下级列表<?php endif; ?></h3>
        <h5>类目添加修改等操作</h5>
      </div>
    </div>
  </div>
...
$(function(){
    $('.flex-table').flexigrid({
        ...
        buttons : [
                   {display: '<i class="fa fa-plus"></i>新增类目', name : 'add', bclass : 'add', onpress : fg_operation },
...
function fg_del(ids) {
    ...
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('<?php echo U("Category/del");?>', {id:id}, function(data){
            if (data.state) {
                location.reload();
            } else {
                showError(data.msg)

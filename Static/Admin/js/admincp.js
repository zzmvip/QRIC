...
//布局换色设置
        ...
        $("div.bgSelector").toggle(300, function() {
            if ($(this).html() == '') {
                $(this).sColor({
                    ...
                    curImg: ADMIN_IMG_URL + '/cur.png',  //必填，颜色选择对象图片路径
    ...
    $('div[data-param^="map-"]').find('i').click(function(){
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON(ADMIN_SITE_URL+'?s=user/common_operations', {value : _value}, function(data){
                if (data) {
...
function openItem(param) {
    ...
    $('#workspace').attr('src', ADMIN_SITE_URL + '?s=' + data_str[1] + '/' + data_str[2]);
